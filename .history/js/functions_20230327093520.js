...
const createStudent = function () {
  ...
  if (pins.includes(createAccountPinInput.value)) {
    ...
    document.getElementById("create-account-pin-error").innerText = "No Pin";
  } else if (createAccountRepinInput.value === "") {
    document.getElementById("create-account-repin-error").innerText =
      "Need to check Pin";
...
const searchStudent = function () {
  let studentsAccessed = JSON.parse(localStorage.getItem(`students`));
  const searchParameter = studentSearchNameInput.value;
  ...
  if (searchResult.length === 0) {
    ...
    informationName.textContent = `Student: ${obj.fullName}`;
    ...
    informationReferals.text =
      obj.referals > 0 ? "Referals: 0" : `Referals: ${obj.referals}`;
    studentSearchNameInput.textContent = "";
  }
};
